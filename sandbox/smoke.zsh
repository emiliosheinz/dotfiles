...
rows=(
    ...
    "python|default|ok|zsh -ic 'python --version' >/dev/null 2>&1"
    "python|default|ok|zsh -ic 'pip --version' >/dev/null 2>&1"
    "python|default|ok|mkdir -p ~/Library/Python/.probe && rmdir ~/Library/Python/.probe; rmdir ~/Library/Python 2>/dev/null; true"
    "python|default|ok|d=\$(mktemp -d); python3 -m venv \$d/v && out=\$(\$d/v/bin/pip install --disable-pip-version-check six 2>&1); rc=\$?; rm -rf \$d; (( rc == 0 )) && ! print -r -- \"\$out\" | grep -q 'cache has been disabled'"
    "tool-caches|default|ok|mkdir -p ~/Library/Caches/.probe && rmdir ~/Library/Caches/.probe"
    "tool-caches|default|fail|touch ~/Library/Application\\ Support/.probe"
